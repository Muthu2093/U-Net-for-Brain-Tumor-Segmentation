...
class dataProcess(object):
	...
	def load_train_data(self):
            print('-'*30)
            print('-'*30)
            #imgs_train = load(self.npy_path+"/train-volume.tif")
            #imgs_mask_train = load(self.npy_path+"/train-labels.tif")
            #imgs_train = imgs_train[0].astype('float32')
            #imgs_mask_train = imgs_mask_train[0].astype('float32')
            
            img = np.zeros([30,512,512])
            for i in range(0,30):
                a = Image.open("data/train/image/%d.tif"%i)
                img[i] = np.array(a)
            imgs_train = img
            imgs_train /= 255
            		#mean = imgs_train.mean(axis = 0)
            		#imgs_train -= mean	
            imgs_train = imgs_train.reshape([30,512,512,1])
            
            img = np.zeros([30,512,512])
            for i in range(0,30):
                a = Image.open("data/train/label/%d.tif"%i)
                img[i] = np.array(a)
            imgs_mask_train = img
            ...
            imgs_mask_train = imgs_mask_train.reshape([30,512,512,1])
	...
	def load_test_data(self):
            ...
            print('-'*30)
            #imgs_test = load(self.npy_path+"/test-volume.tif")
            #imgs_test = imgs_test[0].astype('float32')
            
            img = np.zeros([30,512,512])
            for i in range(0,30):
                a = Image.open("data/test/%d.tif"%i)
                img[i] = np.array(a)
            imgs_test = img
            
            imgs_test /= 255
            imgs_test = imgs_test.reshape([30,512,512,1])
